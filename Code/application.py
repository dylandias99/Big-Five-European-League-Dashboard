...
def all_player_name(season):
    df = input3(season)
    all_names = df.sort_values(by=['Player']).drop_duplicates(subset=['Player'])
    all_teams_name = list(all_names['Player'])
    return all_teams_name
# https://www.linkedin.com/in/dylan-dias-b4bb62150/
# df_new[df_new['l_ext'].isin([31, 22, 30, 25, 64])]
#
# https://github.com/dylandias99


def all_player_nation_name(season):
    df = input3(season)
    all_nation_names = df.sort_values(by=['Nation']).drop_duplicates(subset=['Nation'])
    all_nations_name = list(all_nation_names['Nation'])
    return all_nations_name

# nation = all_player_nation_name()
# new_name = converter.nation
# print(new_name)

def all_squad_name(season):
    df = input3(season)
    all_squad_names = df.sort_values(by=['Squad']).drop_duplicates(subset=['Squad'])
    all_squads_name = list(all_squad_names['Squad'])
    return all_squads_name


def input3(input: int):
    file = "./Player_Stats/Big_5_" + str(input) + ".csv"
    return pd.read_csv(file, parse_dates=True)

# pL = input3(2)
# rel_team = pL[((pL["Nation"].isin([])) & (pL["Pos"].isin()))]
#
# print(rel_team.head())
...
def render_page_content(pathname, season):
    all_names = all_player_name(season)
    all_team = all_squad_name(season)
    all_countries = all_player_nation_name(season)
    if pathname == "/":
        return [
        ...
        return [
            html.H1(['Big 5 European Leagues'], style={'font-style': 'normal'}, className='page-title'),
            html.Div(id='season-container', className="slider-container"),
            html.Div([
                dcc.Dropdown(
                    id='player_name_selector',
                    options=[
                        {'label': i, 'value': i} for i in all_names
                    ],
                    value=[],
                    multi=True
                )
            ], className='player_name_selection'),
            html.Div([
                dcc.Dropdown(
                    id='player_team_selector',
                    options=[
                        {'label': i, 'value': i} for i in all_team
                    ],
                    value=[],
                    multi=True
                )
            ], className='player_team_selection'),
            html.Div([
                dcc.Dropdown(
                    id='player_nation_selector',
                    options=[
            ...
            ], className='player_nation_selection'),
            html.Div([
                dcc.Dropdown(
                    id='player_comp_selector',
                    options=[
            ...
            ], className='player_comp_selector'),
            html.Div([
                dcc.Dropdown(
                    id='player_pos_selector',
                    options=[
            ...
            ], className='player_pos_selector'),
            html.Div(id='player-container', className='player_output'),

        ]


@app.callback(
    dash.dependencies.Output('player-container', 'children'),
    dash.dependencies.Input('season-radio', 'value'),
    dash.dependencies.Input('player_name_selector', 'value'),
    dash.dependencies.Input('player_team_selector', 'value'),
    dash.dependencies.Input('player_nation_selector', 'value'),
    dash.dependencies.Input('player_comp_selector', 'value'),
    dash.dependencies.Input('player_pos_selector', 'value'))
def update_output(season, player, team, nation, comp, position):
    pL = input3(season)
    if player:
        indiv_player_table = pL[pL["Player"].isin(player)]
        return html.Div([
            dash_table.DataTable(
                id='table',
                columns=[{"name": i, "id": i} for i in
                         indiv_player_table.columns],
                data=indiv_player_table.to_dict('records'),
        ...
        indiv_team_table = pL[pL["Squad"].isin(team)]
        return html.Div([
            dash_table.DataTable(
                id='table',
                columns=[{"name": i, "id": i} for i in
                         indiv_team_table.columns],
                data=indiv_team_table.to_dict('records'),
                fixed_rows={'headers': True},
                style_data_conditional=[
                    {
                        'if': {'row_index': 'odd'},
        ...
        indiv_nation_table = pL[pL["Nation"].isin(nation)]
        return html.Div([
            dash_table.DataTable(
                id='table',
                columns=[{"name": i, "id": i} for i in
                         indiv_nation_table.columns],
                data=indiv_nation_table.to_dict('records'),
                fixed_rows={'headers': True},
                style_data_conditional=[
                    {
                        'if': {'row_index': 'odd'},
        ...
        indiv_comp_table = pL[pL["Comp"].isin(comp)]
        return html.Div([
            dash_table.DataTable(
                id='table',
                columns=[{"name": i, "id": i} for i in
                         indiv_comp_table.columns],
                data=indiv_comp_table.to_dict('records'),
                fixed_rows={'headers': True},
                style_data_conditional=[
                    {
                        'if': {'row_index': 'odd'},
        ...
        indiv_pos_table = pL[pL["Pos"].isin(position)]
        return html.Div([
            dash_table.DataTable(
                id='table',
                columns=[{"name": i, "id": i} for i in
                         indiv_pos_table.columns],
                data=indiv_pos_table.to_dict('records'),
                fixed_rows={'headers': True},
                style_data_conditional=[
                    {
                        'if': {'row_index': 'odd'},
